...
from numpy import log, tile, concatenate, arange, meshgrid, ceil, linspace
...
# 提取lfcc特征
def LFCC(sig,
         num_ceps=40,  # 取几个滤波器 40 60
         pre_emph=0,
         pre_emph_coeff=0.97,
         win_len=0.030,  # 0.03 ->480  # 0.06 ->960
         win_hop=0.015,
         win_type="hamming",
         nfilts=70,  # 滤波器的个数，可以调节
         nfft=1024,
         low_freq=None,
         high_freq=None,
         scale="constant",
    ...
    Args:
        # sig            (str) : a mono audio signal (Nx1) from which to compute features.
